...
// Define fetchChatResponse function
const fetchChatResponse = async (query: string, searchResults: string): Promise<string> => {
...
const handleSearch = async () => {
    try {
        ...
        const aiRes = await fetchChatResponse(query, res.data.message);
        setResponse(aiRes);
    } catch (error) {
        console.error('An error occurred while searching the web:', error);
        setSearchResults('An error occurred while searching the web.');
    }
};

// Define handleQueryChange function
                        ...
                        {!messages || messages.length < 1 ? (
                            <Stack className={styles.chatEmptyState}>
                                <img
                                    ...
                                    aria-hidden="true"
                                />
                                <h1 className={styles.chatEmptyStateTitle}>Ask your Compliance Question</h1>
                                <h2 className={styles.chatEmptyStateSubtitle}>This chatbot is configured to answer your questions regarding compliance</h2>
                            </Stack>
                        ) : (
